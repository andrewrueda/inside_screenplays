...
s2v_model = Sense2Vec().from_disk("ScreenPy/s2v/s2v_old")
...
def sense2vec_sim(token1, token2):
	try:
		...
		return cosine_similarity(s2v_model[token1], s2v_model[token2])
...
if DO_TEST:
	...
	try:
		var = SUB.parseString('HELLO GOODBYE An - 3 AM An - HELLO - 3 AM \n')[1]
		print('bad')
	except IndexError:
		print('good')
...
# T
TERIOR = pp.oneOf(['INT.', 'EXT.', 'INT./EXT.', 'EXT./INT.', 'EXT. / INT.', 'INT. / EXT.', 'INT ', 'EXT ']).setResultsName('terior')
