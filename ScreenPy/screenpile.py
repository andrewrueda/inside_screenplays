"""Handle conversion from text-based scripts to json-structured format."""

from ScreenPy.screenpy import *
...
def master_segmentize(segs, verbose=False):
	master_segs = []
	master_seg = []
...
def assemble_lines(text_lines):
	...
	for i, tl in enumerate(text_lines):
		...
		# @andrewrueda: simpler leading_indent
		original = tl
		trimmed = tl.lstrip()
		leading_indent = len(original) - len(trimmed)
		...
		if not start_collecting:
			...
			# @andrewrueda: include INT/EXT without period
			elif 'INT ' in relevant_text[:4] or 'EXT ' in relevant_text[:4]:
				start_collecting = True
		...
		if leading_indent in RANGE_LEFT:
			...
			if is_caps:
				if established_dialogue_indent is not None:
					if leading_indent in range(established_dialogue_indent-2, established_dialogue_indent+2):
...
def annotate_disk(screenplay):
	"""@andrewrueda: Annotate from .txt file to .json file"""
	movie = "_".join(screenplay.split("/")[-1].split(".")[:-1])
	...
	with open(f'test_files/test_json/{movie}.json', 'w') as fp:
		json.dump(masta, fp, indent=4)


def annotate(script_buffer):
	"""@andrewrueda: Annotate from text buffer to json-structured parsed buffer"""
	script_buffer.seek(0)
	script = script_buffer.read()
	...
	# with open(f'titanic_BUFFER.json', 'w') as fp:
	# 	json.dump(masta, fp, indent=4)

	parsed_string = json.dumps(masta)
	parsed_script = StringIO(parsed_string)
	parsed_script.seek(0)
	return parsed_script


if __name__ == '__main__':
	screenplay = "test_files/test_txt/her.txt"
	annotate(screenplay)
